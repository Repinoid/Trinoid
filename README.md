...
## Запускаем контейнер Trino и организуем доступ к S3-хранилищам AWS и Яндекс-облаке
Шаги:
...
### Для этапа разработки включено подробное логирование. Необходимо создать папки и назначить владельца
...
Например, последняя на момент написания apache/hive версия 4.1.0 содержала баг<br>
JARы должны соответствовать друг другу, поэтому лучше сразу не менять на более свежие - **утомитесь** вылавливать ошибки<br>
...
(3 rows)
```
### Создадим таблицу client в схеме mini каталога aws3_catalog, это копия 10 строк таблицы tpch.tiny.customer
```
...
(3 rows)
```
### Создадим таблицу tablo в схеме schematoz каталога ys3_catalog, это копия 3 строк созданной на прошлом шаге таблицы aws3_catalog.mini.client
```
...
(3 rows)
```
Через консоль AWS (или, например, S3 Browser) посмотрите бакет `awstrino` <br>
В нём появилась папка `customer`, в ней файл с именем типа `20250928_135333_00004_3bwb7_13a87dce-4f11-4cb0-87e4-2032947f948b`<br>
В Yandex Cloud Storage также изменения, появился файл `customer/20250928_140259_00008_3bwb7_c3e591a2-609d-4b15-b717-143098f8c2c5`<br>
Это - образы созданных таблиц. <br>
Структура же таблиц и прочая информация хранится в постгрес-базах Metastore<br>

Через SQL запросы в Trino можно работать с S3-хранилищами как с обычными БД<br>
...
Засим,<br>
пишите в Телегу **Telegram @IBM2702**